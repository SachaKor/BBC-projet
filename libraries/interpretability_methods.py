...
def getSenSpeValuesByScoresWeigh(datafram_scores):
    """
    return a a dataframe object with the sensitivity and specificity for all the models in the dataframe scores (tn,fp,fn and tp).
    E.G: (need to be reformatted with the weight value)
    +---------+---------+--------+-----+------+------+-----+-------------------+
    | param_a | param_b | CV_num | tn  |  fp  |  fn  | tp  |     file_name     |
    +---------+---------+--------+-----+------+------+-----+-------------------+
    |     1.0 |     0.0 |    0.0 | 3.0 | 46.0 | 72.0 | 6.0 | conf_A_C ... .ftt |
    ...
    E.G: return more the weigh (index, weight, sen, spe)
    +-------+-------------+-------------+
    | index | Sensitivity | Specificity |
    +-------+-------------+-------------+
    |     0 |    0.076923 |    0.061224 |
    ...
    for index, row in datafram_scores.iterrows():
        ...
        model_file_name = row['file_name']
        sen_spe_values_vec.append([index, weight_values, sensitivity, specificity, model_file_name])
        
    df = pd.DataFrame(sen_spe_values_vec, columns=['index','Weight', 'Sensitivity', 'Specificity', 'file_name'])
    return df

def getMeanSenSpeByWeight(dataframe_values):
    datafram_sen_mean_weigh = dataframe_values.groupby('Weight')['Sensitivity'].mean()
    datafram_spe_mean_weigh = dataframe_values.groupby('Weight')['Specificity'].mean()
    dataframe_mean_sen_spe= pd.concat([datafram_sen_mean_weigh, datafram_spe_mean_weigh], axis=1, sort=False)
    return dataframe_mean_sen_spe


def getMeanSenSpeByNRules(dataframe_values):
    datafram_sen_mean_weigh = dataframe_values.groupby('Weight')['Sensitivity'].mean()
    datafram_spe_mean_weigh = dataframe_values.groupby('Weight')['Specificity'].mean()
    dataframe_mean_sen_spe= pd.concat([datafram_sen_mean_weigh, datafram_spe_mean_weigh], axis=1, sort=False)
    return dataframe_mean_sen_spe
